...
const BulkEdit = {
  findReplace() {
    ...
    if (!findText) { Utils.notify("Please enter text to find", "warning"); return; }
    let count = 0;
  ...
  adjustPrices() {
    ...
    if (isNaN(value)) { Utils.notify("Please enter a valid number", "warning"); return; }
    ProductManager.products.forEach((product) => {
      ...
      switch (operation) {
        ...
        case "multiply": newPrice = currentPrice * (1 + value / 100); break;
      ...
      if (product.variations) {
        product.variations.forEach((variation) => {
          ...
          switch (operation) {
            case "add": newVarPrice = varPrice + value; break;
            case "subtract": newVarPrice = Math.max(0, varPrice - value); break;
            case "multiply": newVarPrice = varPrice * (1 + value / 100); break;
            case "set": newVarPrice = value; break;
          }
  ...
  addTags() {
    ...
    if (newTags.length === 0) { Utils.notify("Please enter tags to add", "warning"); return; }
    ProductManager.products.forEach((product) => {
      if (!product.tags) product.tags = [];
      newTags.forEach((tag) => { if (!product.tags.includes(tag)) product.tags.push(tag); });
    });
    ProductManager.updateUI();
  ...
  // Assign categories to ALL products using the Bulk Edit category tree
  assignCategories() {
    ...
    if (categoryIds.length === 0) { Utils.notify("Please select categories to assign", "warning"); return; }
    ProductManager.products.forEach((product) => {
      if (!product.selectedCategories) product.selectedCategories = [];
      categoryIds.forEach((id) => { if (!product.selectedCategories.includes(id)) product.selectedCategories.push(id); });
    });
    ProductManager.updateUI();
  ...
  deleteSelected() {
    if (ProductManager.selectedIndices.length === 0) { Utils.notify("No products selected", "warning"); return; }
    if (!confirm(`Delete ${ProductManager.selectedIndices.length} selected products?`)) return;
  ...
  // IMPLEMENTED: Assign categories to SELECTED products (Merge/Replace) via modal
  assignCategoriesToSelected() {
    if (ProductManager.selectedIndices.length === 0) { Utils.notify("Select some products first", "warning"); return; }
  ...
  applyCategoryAssign(selectedOnly = true) {
    ...
    const categoryIds = Array.from(checkboxes).map((cb) => parseInt(cb.value));
    if (categoryIds.length === 0) { Utils.notify("Please select at least one category", "warning"); return; }
    ...
    closeModal();
    Utils.notify(
      `✓ Assigned ${categoryIds.length} categor${categoryIds.length === 1 ? "y" : "ies"} to ${updated} product${updated === 1 ? "" : "s"} (${replace ? "replaced" : "merged"})`,
      "success"
    );
  },
};
